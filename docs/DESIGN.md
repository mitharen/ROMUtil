...
4. **Disjunctive Dummy Stub Collision Avoidance**:
   Because boundary dummy rooms are omitted from $m.\text{Rooms}$ and represented as affine linear expressions $\mathbf{x}_{\text{dummy}} = \mathbf{x}_s + \mathbf{d}_{\text{exit}}$, core rooms could theoretically occupy identical spatial coordinates to an adjacent dummy stub if unconstrained. During iterative collision detection, room-to-dummy point collisions ($\mathbf{x}_v = \mathbf{x}_s + \mathbf{d}$) are identified via direct $O(1)$ spatial coordinate hash table lookup against core room spatial buckets ($O(D)$ total time across all dummy stubs) and resolved by generating disjunctive Big-M separation constraints:
...
5. **Disjunctive Core Room Point Collision Avoidance**:
   In layouts containing parallel topological paths, converging exit funnels, or disconnected graph components, distinct core rooms $u, v \in V_{\text{core}}$ ($u < v$) can occupy identical integer coordinates ($\mathbf{x}_u = \mathbf{x}_v$) in intermediate solver relaxations. During iterative collision detection in `solve()`, core rooms are partitioned into spatial coordinate buckets in $O(V)$ time (`build_spatial_coordinate_buckets`), and pairwise candidate collisions are generated strictly within multi-room collision buckets ($\text{len}(vnums) > 1$) using `find_spatial_room_collisions` in $O(V)$ average time before candidate cut generation and batch capping. Coincident core rooms are separated by generating pairwise disjunctive separation constraints:
...
6. **Disjunctive Exit-Through-Room (Collinear Penetration) Collision Avoidance**:
   An elongated exit segment $e = (u, v)$ with distance $> 1$ can collinear-penetrate an intermediate third-party core room $w \in V_{\text{core}} \setminus \{u, v\}$ located strictly along the corridor line segment between $u$ and $v$ ($\mathbf{x}_w = \mathbf{x}_u + t(\mathbf{x}_v - \mathbf{x}_u)$ for $t \in (0, 1)$). During iterative collision detection in `solve()`, candidate penetrations are identified in $O(E)$ time via rasterized integer coordinate queries against spatial coordinate hash buckets (`coords_buckets`) using greatest common divisor step resolution $g = \gcd(|\Delta x|, |\Delta y|, |\Delta z|)$ along exit displacement vectors. When $g > 1$, intermediate integer grid positions $\mathbf{x}_u + k \frac{\Delta \mathbf{x}}{g}$ for $k \in \{1, \dots, g - 1\}$ are evaluated against spatial room buckets in $O(1)$ time. Penetrating rooms are resolved by adding disjunctive Big-M separation constraints that enforce either perpendicular displacement of room $w$ relative to the exit segment or longitudinal clearance outside the corridor segment:
   $$x_p - x_w + M_{\text{sep}}^x ((1 - \text{relation}_{\text{east}}) + \text{cut}_e) \ge d_{\min}, \quad \forall p \in \{u, v\}$$
   $$x_w - x_p + M_{\text{sep}}^x ((1 - \text{relation}_{\text{west}}) + \text{cut}_e) \ge d_{\min}, \quad \forall p \in \{u, v\}$$
   $$y_p - y_w + M_{\text{sep}}^y ((1 - \text{relation}_{\text{north}}) + \text{cut}_e) \ge d_{\min}, \quad \forall p \in \{u, v\}$$
   $$y_w - y_p + M_{\text{sep}}^y ((1 - \text{relation}_{\text{south}}) + \text{cut}_e) \ge d_{\min}, \quad \forall p \in \{u, v\}$$
   $$z_p - z_w + M_{\text{sep}}^z ((1 - \text{relation}_{\text{up}}) + \text{cut}_e) \ge d_{\min}, \quad \forall p \in \{u, v\}$$
   $$z_w - z_p + M_{\text{sep}}^z ((1 - \text{relation}_{\text{down}}) + \text{cut}_e) \ge d_{\min}, \quad \forall p \in \{u, v\}$$
   $$\sum_{d \in \mathcal{D}_{\text{active}}} \text{relation}_d \ge 1, \quad \text{relation}_d \in \{0, 1\}$$
   where dimension-specific bounds $M_{\text{sep}}^d = 2 M_d + 4$ guarantee mathematical feasibility. When the exit is cut ($\text{cut}_e = 1$), the constraint is relaxed without creating rendering artifacts. When planar ($\text{has\_vertical\_exits} = \text{False}$), $\mathcal{D}_{\text{active}} = \{\text{North}, \text{East}, \text{South}, \text{West}\}$, eliminating vertical decision variables. In 3D topologies ($\text{has\_vertical\_exits} = \text{True}$), $\mathcal{D}_{\text{active}}$ spans all 6 spatial directions to support orthogonal floor separation. Constraints are batched iteratively according to candidate batch caps alongside core room, dummy stub, and line crossing collision passes to guarantee bounded branch-and-cut solve times.
